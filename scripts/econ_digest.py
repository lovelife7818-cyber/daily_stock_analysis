...
严格按下面格式输出 Markdown，不要额外标题、不要代码块：

LINE1: 一句话（40-70 字）—— 今天公布的数据整体偏强 / 偏弱 / 中性，以及它把市场往哪个方向推。
LINE2: 一句话（30-55 字）—— 这些数据对「美股科技成长股 + 加密概念股」意味着什么。
DATA:
- 数据名｜实际值｜预期值｜一句话影响（最多 4 条；没有数字的条目不要写）
PENDING:
- 待公布数据名｜北京时间几点｜为什么值得看（最多 3 条；没有就写「无」）

硬性约束：
1. 【最重要】**只允许使用上面检索结果里出现过的数字**。任何不在材料里的数字一律不许出现。
   如果某条数据只找到名字、没找到实际值，就只写名字，实际值写「未获取」。
2. 完全找不到当天经济数据，就 LINE1 写「今日未检索到明确的经济数据发布」，DATA 写「- 无」。
3. 不要写免责声明，不要写「综上所述」。
4. 数据名用中文（如「8月CPI同比」），保留原始数值格式。"""
